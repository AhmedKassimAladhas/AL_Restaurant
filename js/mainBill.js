...
$(document).ready(function() {
		...
		if (bdelevery == "" & discount == "") {
			...
			$("#bill_total").val(parseFloat(bTotal)+ parseFloat(bdelevery));
		} else if (bdelevery == "" & discount != "") {
			$("#bill_total").val(parseFloat(bTotal)- parseFloat(discount));
		} else {
			$("#bill_total").val(parseFloat(bTotal)+ parseFloat(bdelevery)- parseFloat(discount));
		}
	})
});
$(document).ready(function() {
	$("#Dbill_discount").change(function() {
		var discount = $("#Dbill_discount").val();
		var bTotal = $("#bill_total_Sale").val();
		var bdelevery = $("#delevery_cost").val();
...
$(document).ready(function() {
	$("#cust_phone").change(function() {
		...
		var url = "http://localhost:8081/ALRestaurant/getCustomer?cust_phone="+cust_phone;
		...
		function myFunction(arr) {	
			  ...
			  $("#cust_address").val(arr.cust_address);
			  $("#cust_notes").val(arr.cust_notes);
...
function saveCust() {
	...
	var xmlhttp = new XMLHttpRequest();
	var url = "http://localhost:8081/ALRestaurant/saveCustomer?cu_id="+cust_id+"&cust_name="+cust_name+"&cust_phone="+cust_phone+"&cust_address="+cust_address+"&cust_notes="+cust_notes;
...
delevery.addEventListener('show.bs.modal', function(event) {
	...
	var cust_id = delevery.querySelector('.modal-body #cust_id')
	t.value = btype
	i.value = id
	b_id.value = id
	b_num.value = bnum
...
var addNew = document.getElementById('addnewsmallModel')
addNew.addEventListener('show.bs.modal', function(event) {
	var button = event.relatedTarget
	var id = button.getAttribute('data-bs-id')
	var menBuy = button.getAttribute('data-bs-buy')
	var menSale = button.getAttribute('data-bs-sale')
	var menName = button.getAttribute('data-bs-name')

	var men_id = addNew.querySelector('.modal-body #men_id')
	var men_buy = addNew.querySelector('.modal-body #men_buy')
	var men_sale = addNew.querySelector('.modal-body #men_sale')
	var men_name = addNew.querySelector('.modal-body #men_name')
	men_id.value = id
...
exampleModal.addEventListener('show.bs.modal', function(event) {
	...
	var Lineid = exampleModal.querySelector('.modal-body input')
...
takeAwy.addEventListener('show.bs.modal', function(event) {
	var button = event.relatedTarget
	var id = button.getAttribute('data-bs-id')
	var btN = button.getAttribute('data-bs-btN')
	var btype = button.getAttribute('data-bs-type')
	var bBuy = button.getAttribute('data-bs-buy')
	var bSale = button.getAttribute('data-bs-sale')
	...
	var bt_num = takeAwy.querySelector('.modal-body #bill_number')
